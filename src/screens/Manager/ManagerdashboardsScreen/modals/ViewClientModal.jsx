...
const ViewBillModal = ({ isVisible, onClose, billData, client }) => {
  ...
  // Remove debug logs and use clean logic
  const notes = billData?.notes || 'No notes provided';
  const beautician = billData?.beautician || 'Not assigned';

  // Additional debug to check if we're getting actual user input
  console.log('=== FINAL DISPLAY VALUES ===');
  console.log('Notes for display:', notes);
  console.log('Beautician for display:', beautician);
