...
const AdminAuthGate = ({ navigation }) => {
    ...
    const [isAppRegisteredInitially, setIsAppRegisteredInitially] = useState(false);

    useEffect(() => {
        const performInitialCheck = async () => {
            if (!isLoading) { // Ensure UserContext has finished its own loading
    ...
    useEffect(() => {
        if (isInitialCheckDone) {
            if (!isAppRegisteredInitially) {
                ...
                console.log("AdminAuthGate: App not registered, navigating to AdminRegister");
                navigation.replace('AdminRegister');
            } else {
                // Scenario 2: App IS registered (any subsequent launch) -> Always go to Login Screen
                console.log("AdminAuthGate: App is registered, navigating to AdminLogin to re-authenticate");
