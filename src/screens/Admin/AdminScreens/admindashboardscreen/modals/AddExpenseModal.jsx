...
const AddExpenseModal = ({ isVisible, onClose, onSave }) => {
    ...
    const handleSave = () => {
        if (!expenseName.trim() || !amount.trim() || !description.trim()) {
            showCustomAlert('Please fill all fields: Expense Name, Amount, and Description.');
            return;
        }
        ...
        const newExpense = {
            ...
            image: imageUri, // <--- IMPORTANT: Changed from imageUri to image to match ViewExpenseModal
        };
    ...
    // --- IMPORTANT: Updated handleImagePicker to use async/await ---
    const handleImagePicker = async () => {
        ...
        try {
            const response = await launchImageLibrary(options); // Use await here

            if (response.didCancel) {
                console.log('User cancelled image picker');
            } else if (response.errorCode) {
                console.log('ImagePicker Error: ', response.errorCode, response.errorMessage);
                showCustomAlert(`Image Picker Error: ${response.errorMessage || 'Something went wrong.'}`);
            ...
            showCustomAlert('Failed to open image library. Please check permissions.');
        }
    };
    ...
    return (
        ...
        >
            ...
            {/* Custom Alert Modal (local to AddExpenseModal) */}
            <Modal
                animationType="fade"
                transparent={true}
